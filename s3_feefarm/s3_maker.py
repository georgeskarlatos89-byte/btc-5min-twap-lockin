...
class T:
    def __init__(self):
        ...
        self.liq_trig_ts = 0.0                 # newest cascade already acted on (no re-fire on stale rows)
        self.hb_t = 0.0                        # heartbeat: tick_ts in stats every 60 s
...
def gates(now):
    ...
    if HAVE_S8:
        s8_allowed, s8_reason, _ = s8_router.evaluate_strategy_gate("S3", now, current_sigma=sigma5m_bps())
        if not s8_allowed:
...
def scan_cascade(rows):
    ...
    j = 0
    hit = None                                   # keep the LATEST crossing so a new cascade behind an
    for i in range(len(btc)):                    # older one in the 10-min window still fires
...
async def moondev_loops():
    ...
    while True:
        ...
        try:
            ...
            # 2026-09-24: the feed keeps a cascade for 10 min, so the same event re-pulled every poll
            # once the 60 s lock expired (10 "LIQ" pulls, all the same cascade). Fire once per cascade.
...
def do_pull(reason):
    ...
    csv_append(PULLS_CSV, ["ts", "reason", "active_rounds"],
               [int(now), reason, sum(1 for r in t.rounds.values() for q in r["quotes"].values()
                                      if q["phase"] == "RESTING" and q.get("placed"))])
...
def place_quotes(rs, now, live):
    ...
    if global_gates:
        ...
        if not rs.get("skip_logged"):              # once per round: why we are flat (was invisible)
...
async def tick_loop():
    while True:
        # 2026-09-24: guarded - an unhandled exception here ended the strategy while systemd
        # still showed "active". Heartbeat (stats["tick_ts"]) lets the monitor detect a dead loop.
        try:
            if time.time() - t.hb_t >= 60:
                t.hb_t = time.time(); t.stats["tick_ts"] = int(t.hb_t); save_stats()
            now = time.time()
...
def preflight():
    ...
    if HAVE_S8:
        ...
        pol = st8["strategy_policies"].get("S3", {})
