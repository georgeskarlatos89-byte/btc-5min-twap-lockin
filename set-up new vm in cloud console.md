...
## Part 8 — Increase the disk (storage) later, with no downtime

Needed on 2026-09-21: the order-book recorder writes ~2.2 GB/day, and the default 20 GB disk had
only 15 GB free. **The path that works in the console:**

1. **Compute Engine → left sidebar → Storage → Disks.**
2. Click the disk **name** in the list (the first clickable link — e.g. `arena-ai-bots`).
3. On the **Manage disk** page, click **Edit** (pencil) in the **top toolbar**.
4. Change **Size** (e.g. `20` → `50`) → **Save**. Size can only go **up**.

**Traps:** on the VM *instance* Edit form the boot-disk name is **not clickable** and the size is
not editable there. **Additional disks → Add new disk** is the wrong path: it creates a second,
blank disk that must be formatted, mounted and have data moved onto it.
CLI alternative (Cloud Shell `>_`): `gcloud compute disks resize <disk> --size=50GB --zone=<zone>`

**Then make Ubuntu use the space (live, no reboot, services keep running):**
```bash
ssh twapvm 'sudo growpart /dev/sda 1 && sudo resize2fs /dev/sda1 && df -h /'
```
(Root is ext4 on `sda1` in the GCP Ubuntu 24.04 image — confirm with `lsblk` / `findmnt /`.
Until this runs, `df` still shows the old size.)
...
| thing | value |
|---|---|
| **New TWAP VM** | `arena-ai-bots`, europe-west4-a, IP `34.34.13.7`, alias `twapvm`, user `ubuntupolymarket3`; boot disk **50 GB** since 2026-09-21 (was 20) |
