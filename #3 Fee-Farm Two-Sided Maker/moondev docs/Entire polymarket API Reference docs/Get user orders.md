...
> Retrieves live orders for the authenticated user. Returns paginated results.
Filtering by id returns that order regardless of status, including canceled or fully matched orders.
...
paths:
  /data/orders:
    get:
      ...
      description: >
        Retrieves live orders for the authenticated user. Returns paginated
        results.

        Filtering by id returns that order regardless of status, including
        canceled or fully matched orders.
    ...
    OpenOrder:
      ...
      properties:
        ...
        status:
          type: string
          description: >-
            Order status. CANCELED_MARKET_RESOLVED cancels the unfilled
            remainder when the market resolves; it does not undo settled fills.
          enum:
            - LIVE
            - INVALID
        ...
        original_size:
          type: string
          description: >-
            Original order size in shares, as a decimal string. Already
            normalized; do not divide by 1e6.
          example: '100'
        size_matched:
          type: string
          description: >-
            Matched size in shares, as a decimal string. Retained after
            cancellation; this is not the current position balance.
          example: '0'
        price:
          type: string
